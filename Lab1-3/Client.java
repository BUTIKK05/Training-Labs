...
public class Client extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        store = (StoreService) LocateRegistry.getRegistry("26.135.102.136", 1099).lookup("store");
        ...
        showProductsBtn.setOnAction(e -> {
            try {
                String warehouseNameInput = warehouseName.getText();
                var products = store.getProductsByWarehouseName(warehouseNameInput);
                if (products.isEmpty()) {
                    showAlert(Alert.AlertType.INFORMATION, "Нет товаров на складе: " + warehouseNameInput);
                } else {
                    StringBuilder sb = new StringBuilder("Товары на складе " + warehouseNameInput + ":\n");
                    for (var p : products) {
        ...
        root.getChildren().addAll(
                ...
                new Label("Имя склада (куда/откуда товар):"), warehouseName,
                new Label("Процент изменения цены:"), percent,
